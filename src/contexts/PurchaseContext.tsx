...
export function PurchaseProvider({ children }: { children: ReactNode }) {
  // iOS paid app = everything unlocked
  const isIOSPaidApp = Platform.OS === 'ios';
  
  const [isPremium, setIsPremium] = useState(isIOSPaidApp);
  const [isSubscribed, setIsSubscribed] = useState(isIOSPaidApp);
  const [hasRemovedAds, setHasRemovedAds] = useState(isIOSPaidApp);
  const [purchasedThemes, setPurchasedThemes] = useState<string[]>(isIOSPaidApp ? PREMIUM_THEMES : []);
