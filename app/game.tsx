...
// Use larger board for tablets/iPads
const isTablet = width > 768;
const BOARD_SIZE = isTablet ? Math.min(width * 0.7, 800) : Math.min(width - 32, 400);
...
export default function GameScreen() {
  ...
  const loadDLCSettings = async () => {
    try {
      // Diarrhea Word Set is ONLY available on WEB - not in native apps
      // This makes the apps suitable for all ages
      ...
      if (purchases) {
        ...
        // Diarrhea DLC only on web, not native apps
        const hasDiarrheaDLC = !isNativeApp && (purchaseList.includes(DLC_IDS.DIARRHEA_SET) || hasPremium);
        ...
        // Diarrhea DLC only works on web
        setUseDiarrheaDLC(!isNativeApp && hasDiarrheaDLC && useDLC === 'true');
