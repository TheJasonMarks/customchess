...
export default function SettingsScreen() {
  ...
  // Diarrhea Word Set is ONLY available from website download - NOT in apps
  // This makes the app suitable for all ages
  const isIOSPaid = Platform.OS === 'ios';
  const isNativeApp = Platform.OS === 'ios' || Platform.OS === 'android';
  
  const hasDiarrheaDLC = () => {
    // Diarrhea DLC only available on WEB (website download)
    // Not available in native apps to keep it all-ages friendly
  ...
  const hasOverlays = () => {
    return isIOSPaid ||
           webPurchases.includes('letter_overlays') || 
  ...
  const hasCornerLetters = () => {
    return isIOSPaid ||
           webPurchases.includes('corner_letters') ||
           webPurchases.includes('premium_unlock') || 
           webPurchases.includes('premium_plus');
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: currentTheme.background }]}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Crude Humor 13+ Toggle - ONLY show on WEB, not in native apps */}
        {Platform.OS === 'web' && (
          <View style={styles.section}>
